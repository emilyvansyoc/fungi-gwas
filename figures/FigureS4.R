...
####### PRJNA541487 #######
load("data/publicITS_phyloseqs//raw_phyloseq_PRJNA541487.RData")
psf_541487 <- ps %>%
...
meta_541487 <- readxl::read_xlsx("data/publicITS_phyloseqs//Sra_PRJNA54187.xlsx") %>% 
...
##### PRJNA703732 ####
load("data/publicITS_phyloseqs///raw_phyloseq_PRJNA703732.RData")
psf_703732 <- ps %>% tax_fix() %>% tax_select("Fungi", "Kingdom")
## get just healthy controls
meta_703732 <- read.table("data/publicITS_phyloseqs//Sra_PRJNA703732.csv", sep = ",", header = TRUE) %>% column_to_rownames(var = "Run")
...
##### PRJNA698272 #####
load("data/publicITS_phyloseqs//raw_phyloseq_PRJNA698272.RData")
psf_698272 <- ps %>% tax_fix() %>% tax_select("Fungi", "Kingdom")
### get just healthy people
meta698272 <- read.table("data/publicITS_phyloseqs/Sra_PRJNA698272.csv", sep = ",", header = TRUE) %>% column_to_rownames(var = "Run")
...
load("data/publicITS_phyloseqs//raw_phyloseq_PRJEB11419.RData")
...
#plot_610042 <- plot_610042 + mymargin
...
ggarrange(plot_541487, #plot_610042, 
          ...
          labels = c(
            "A. PRJNA541487 (Chinese)", #"B. PRJNA610042 (American)",
...
# save
ggsave(filename = "figures/FigureS4.png", dpi = 300, height = 12, width = 16, units = "in")
